...
public class Tensorforia extends VuforiaLocalizerImpl {
    ...
    public Tensorforia(Parameters parameters) {
        super(parameters);
    }

    public Bitmap getBitmapFromRenderQueue(){
        if(!getFrameQueue().isEmpty()){
            try {
                bitmap = convertFrameToBitmap(getFrameQueue().take());
    ...
    public void start(){
        ...
        workerThread.setName("Tensorforia Thread");
        workerThread.start();

        Log.d("TensorCV", workerThread.getState().toString());
